...
UPSAMPLE_VIDEO_URL = 'https://aisandbox-pa.googleapis.com/v1/video:batchAsyncGenerateVideoUpsampleVideo'
...
def get_reference_video_model_key(aspect_ratio: str, tier: str = 'PAYGATE_TIER_TWO', model_speed: str = 'relaxed') -> str:
    ...
    return 'veo_3_1_r2v_fast_portrait' if is_portrait else 'veo_3_1_r2v_fast_landscape'
...
class FlowBrowser:
    ...
    def _spawn_chrome(self, chrome_path: str) -> str:
        self._debug_port = random.randint(9222, 10222)
        args = [
            chrome_path,
    ...
    async def _evaluate_create_image_internal_async(self, config: dict) -> dict:
        page = await self._ensure_internal_async()
        lane_key = str(config.get('laneId') or self.lane_id or 'default')
        self._log(
            f'[Flow:{self.lane_id}] lane={lane_key} browser-evaluate-start '
            f'action={config.get("action")} url={config.get("apiUrl")}'
        )
        try:
            result = await asyncio.wait_for(
                page.evaluate(EXECUTE_IMAGE_JS, config),
                timeout=60.0,
            )
        except asyncio.TimeoutError as exc:
            self._log(
                f'[Flow:{self.lane_id}] lane={lane_key} browser-evaluate-timeout '
                f'timeout=60s action={config.get("action")} url={config.get("apiUrl")}'
            )
            self._page = None
            raise BananaClientError('Browser evaluate timeout after 60s') from exc
        self._log(
            f'[Flow:{self.lane_id}] lane={lane_key} browser-evaluate-ok '
            f'action={config.get("action")} url={config.get("apiUrl")}'
        )
        return result
...
class BananaImageClient:
    ...
    def upscale_video_1080p(
        ...
        data = result.get('data') or {}
        operations = data.get('operations') or []
        operation = (operations[0].get('operation') or operations[0] or {}) if operations else (data.get('operation') or {})
        operation_name = (
            data.get('mediaGenerationId')
        ...
        download_url = (
            ...
            or data.get('fifeUrl')
            or (((data.get('media') or [{}])[0].get('video') or {}).get('fifeUrl') if isinstance(data.get('media'), list) else None)
        )
        if download_url:
            return {'type': 'sync', 'downloadUrl': download_url, 'raw': data}
        raise BananaClientError('No video 1080p upsample operation or URL found in response')
    ...
    def poll_video(self, access_token: str, media_name: str, project_id: Optional[str] = None) -> dict:
        body = {
            'media': [
                {
                    'name': media_name,
        ...
        media_item = media_items[0] or {}
        media_metadata = media_item.get('mediaMetadata') or {}
        media_status = media_metadata.get('mediaStatus') or {}
        status = media_status.get('mediaGenerationStatus')
        if status == 'MEDIA_GENERATION_STATUS_SUCCESSFUL':
            return {'done': True, 'failed': False, 'raw': data}
        if status == 'MEDIA_GENERATION_STATUS_FAILED':
            self._log(f'stage=poll-video-failed-raw response={json.dumps(data, ensure_ascii=False)}')
            return {'done': True, 'failed': True, 'raw': data}
        return {'done': False, 'failed': False, 'raw': data}
